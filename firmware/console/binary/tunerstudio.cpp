 ...
 *
 * queryCommand:
 * 		Communication initialization command. TunerStudio sends a single
 * byte H ECU response: One of the known ECU id strings.
 *
 * ochGetCommand:
 * 		Request for output channels state.TunerStudio sends a single
 * byte O ECU response: A snapshot of output channels as described in
 * [OutputChannels] section of the .ini file The length of this block is
 * 'ochBlockSize' property of the .ini file
 *
 * These two commands are enough to get working gauges. In order to start
 * configuring the ECU using tuner studio, three more commands should be
 * implemented:
 *
 * See also
 ...
 *
 * rusEfi is free software; you can redistribute it and/or modify it under the
 * terms of the GNU General Public License as published by the Free Software
 * Foundation; either version 3 of the License, or (at your option) any later
 * version.
 *
 * rusEfi is distributed in the hope that it will be useful, but WITHOUT ANY
...
// We have TS protocol limitation: offset within one settings page is uin16_t
// type.
static_assert(sizeof(*config) <= 65536);

static void printErrorCounters() {
  efiPrintf("TunerStudio size=%d / total=%d / errors=%d / H=%d / O=%d / P=%d / "
            "B=%d / 9=%d",
            sizeof(engine->outputChannels), tsState.totalCounter,
            tsState.errorCounter, tsState.queryCommandCounter,
            tsState.outputChannelsCommandCounter,
            tsState.readPageCommandsCounter, tsState.burnCommandCounter,
            tsState.readScatterCommandsCounter);
  efiPrintf("TunerStudio C=%d", tsState.writeChunkCommandCounter);
  efiPrintf("TunerStudio errors: underrun=%d / overrun=%d / crc=%d / "
            "unrecognized=%d / outofrange=%d / other=%d",
            tsState.errorUnderrunCounter, tsState.errorOverrunCounter,
            tsState.errorCrcCounter, tsState.errorUnrecognizedCommand,
            tsState.errorOutOfRange, tsState.errorOther);
}
...
static void resetTs() { memset(&tsState, 0, sizeof(tsState)); }
...
static uint8_t *getWorkingPageAddr(TsChannelBase *tsChannel, size_t page,
                                   size_t offset) {
...
#endif
  default:
    // technical dept: TS seems to try to read the 3 pages sequentially, does
    // not look like we properly handle 'EFI_TS_SCATTER=FALSE'
...
// Validate whether the specified offset and count would cause an overrun in the
// tune. Returns true if an overrun would occur.
static bool validateOffsetCount(size_t page, size_t offset, size_t count,
  ...
  if (offset + count > allowedSize) {
    efiPrintf("TS: Project mismatch? Too much configuration requested %d+%d>%d",
              offset, count, allowedSize);
...
PUBLIC_API_WEAK bool isBoardAskingTriggerTsRefresh() { return false; }

bool needToTriggerTsRefresh() {
  return !engine->engineTypeChangeTimer.hasElapsedSec(1);
}

void onApplyPreset() { engine->engineTypeChangeTimer.reset(); }

PUBLIC_API_WEAK bool isTouchingVe(uint16_t offset, uint16_t count) {
  return isTouchingArea(offset, count, offsetof(persistent_config_s, veTable),
                        sizeof(config->veTable));
}
...
bool isTouchingArea(uint16_t offset, uint16_t count, int areaStart,
                    int areaSize) {
...
void TunerStudio::handleWriteChunkCommand(TsChannelBase *tsChannel,
  ...
  if (page == TS_PAGE_SETTINGS) {
    ...
    // Skip the write if a preset was just loaded - we don't want to overwrite
    // it [tag:popular_vehicle]
    if (!needToTriggerTsRefresh()) {
      memcpy(addr, content, count);
    } else {
      efiPrintf("Ignoring TS -> Page %d write chunk offset %d count %d "
                "(output_count=%d)",
                page, offset, count, tsState.outputChannelsCommandCounter);
    }
    // Force any board configuration options that humans shouldn't be able to
    // change huh, why is this NOT within above 'needToTriggerTsRefresh()'
    // condition?
...
void TunerStudio::handleCrc32Check(TsChannelBase *tsChannel, uint16_t page,
  ...
  efiPrintf("TS <- Get CRC page %d offset %d count %d result %08x", page,
            offset, count, (unsigned int)crc);
...
//	printf("totalResponseSize %d\n", totalResponseSize);
  ...
  // Command part of CRC
  uint32_t crc =
      tsChannel->writePacketHeader(TS_RESPONSE_OK, totalResponseSize);
...
void TunerStudio::handlePageReadCommand(TsChannelBase *tsChannel, uint16_t page,
                                        uint16_t offset, uint16_t count) {
...
static void handleBurnCommand(TsChannelBase *tsChannel, uint16_t page) {
  if (page == TS_PAGE_SETTINGS) {
    ...
    // problem: 'popular vehicles' dialog has 'Burn' which is very NOT helpful
    // on that dialog since users often click both buttons producing a conflict
    // between ECU desire to change settings and TS desire to send TS
    // calibration snapshot into ECU Skip the burn if a preset was just loaded -
    // we don't want to overwrite it [tag:popular_vehicle]
    if (!needToTriggerTsRefresh()) {
...
static bool isKnownCommand(char command) {
  return command == TS_HELLO_COMMAND || command == TS_READ_COMMAND ||
         command == TS_OUTPUT_COMMAND || command == TS_BURN_COMMAND ||
         ...
         command == TS_GET_FIRMWARE_VERSION || command == TS_PERF_TRACE_BEGIN ||
         command == TS_PERF_TRACE_GET_BUFFER ||
         command == TS_GET_CONFIG_ERROR || command == TS_QUERY_BOOTLOADER;
}
...
static void handleTestCommand(TsChannelBase *tsChannel) {
  ...
  tsChannel->write((const uint8_t *)QUOTE(SIGNATURE_HASH),
                   sizeof(QUOTE(SIGNATURE_HASH)));

  chsnprintf(testOutputBuffer, sizeof(testOutputBuffer), " %d %d",
             engine->engineState.warnings.lastErrorCode,
             tsState.testCommandCounter);
  tsChannel->write((const uint8_t *)testOutputBuffer, strlen(testOutputBuffer));

  chsnprintf(testOutputBuffer, sizeof(testOutputBuffer), " uptime=%ds ",
             (int)getTimeNowS());
  tsChannel->write((const uint8_t *)testOutputBuffer, strlen(testOutputBuffer));
  ...
  if (hasFirmwareError()) {
    ...
    chsnprintf(testOutputBuffer, sizeof(testOutputBuffer), "error=%s\r\n",
               error);
    tsChannel->write((const uint8_t *)testOutputBuffer,
                     strlen(testOutputBuffer));
  }
  tsChannel->flush();
}
...
void TunerStudio::handleQueryCommand(TsChannelBase *tsChannel,
  ...
  tsChannel->sendResponse(mode, (const uint8_t *)signature,
                          strlen(signature) + 1);
}
...
bool TunerStudio::handlePlainCommand(TsChannelBase *tsChannel,
  ...
  if (command == 0) {
    ...
    // We interpret 'Q' as TS_HELLO_COMMAND, since TS uses hardcoded 'Q' during
    // ECU detection (scan all serial ports)
     ...
     * "The F command is used to find what ini. file needs to be loaded in
     * TunerStudio to match the controller. If you are able to just make your
     * firmware ignore the command that would work. Currently on some firmware
     * versions the F command is not used and is just ignored by the firmware as
     * a unknown command."
     */
...
static int tsProcessOne(TsChannelBase *tsChannel) {
  assertStack("communication", ObdCode::STACK_USAGE_COMMUNICATION,
              EXPECTED_REMAINING_STACK, -1);
  ...
  uint8_t firstByte;
  size_t received =
      tsChannel->readTimeout(&firstByte, 1, TS_COMMUNICATION_TIMEOUT);
#if EFI_SIMULATOR
  logMsg("received %d\r\n", received);
#endif // EFI_SIMULATOR
  ...
  received =
      tsChannel->readTimeout(&secondByte, 1, TS_COMMUNICATION_TIMEOUT_SHORT);
  if (received != 1) {
  ...
  if ((incomingPacketSize == 0) ||
      (expectedSize > sizeof(tsChannel->scratchBuffer))) {
    if (tsChannel->in_sync) {
      efiPrintf("process_ts: channel=%s invalid size: %d", tsChannel->name,
                incomingPacketSize);
  ...
  char command;
  if (tsChannel->in_sync) {
    /* we are in sync state, packet size should be correct so lets receive full
     * packet and then check if command is supported otherwise (if abort
     * reception in middle of packet) it will break synchronization and cause
     * error on next packet */
    received = tsChannel->readTimeout((uint8_t *)(tsChannel->scratchBuffer),
                                      expectedSize, TS_COMMUNICATION_TIMEOUT);
    command = tsChannel->scratchBuffer[0];

    if (received != expectedSize) {
      /* print and send error as we were in sync */
      efiPrintf("Got only %d bytes while expecting %d for command 0x%02x",
                received, expectedSize, command);
      tunerStudioError(tsChannel, "ERROR: not enough bytes in stream");
      // MS serial protocol spec: There was a timeout before all data was
      // received. (25ms per character.)
    ...
    received = tsChannel->readTimeout((uint8_t *)(tsChannel->scratchBuffer), 1,
    ...
    received =
        tsChannel->readTimeout((uint8_t *)(tsChannel->scratchBuffer) + 1,
                               expectedSize - 1, TS_COMMUNICATION_TIMEOUT);
  ...
  uint32_t expectedCrc =
      *(uint32_t *)(tsChannel->scratchBuffer + incomingPacketSize);
  ...
  if (actualCrc != expectedCrc) {
    /* send error only if previously we were in sync */
    if (tsChannel->in_sync) {
      efiPrintf("TunerStudio: command %c actual CRC %x/expected %x",
                tsChannel->scratchBuffer[0], (unsigned int)actualCrc,
  ...
  int success = tsInstance.handleCrcCommand(tsChannel, tsChannel->scratchBuffer,
                                            incomingPacketSize);
...
#ifndef CUSTOM_BRAND_NAME
#define CUSTOM_BRAND_NAME FRONTEND_TITLE_BAR_NAME
#endif
  char versionBuffer[32];
  chsnprintf(versionBuffer, sizeof(versionBuffer), "%s v%d@%u",
             CUSTOM_BRAND_NAME, getRusEfiVersion(), SIGNATURE_HASH);
  tsChannel->sendResponse(TS_CRC, (const uint8_t *)versionBuffer,
                          strlen(versionBuffer) + 1);
}
...
static void handleGetText(TsChannelBase *tsChannel) {
  ...
  tsChannel->writeCrcPacket(TS_RESPONSE_OK,
...
void TunerStudio::handleExecuteCommand(TsChannelBase *tsChannel, char *data,
...
int TunerStudio::handleCrcCommand(TsChannelBase *tsChannel, char *data,
                                  int incomingPacketSize) {
  ...
  // not used by .ini at the moment TODO actually use that version of the
  // command in the .ini
  ...
  case TS_OUTPUT_ALL_COMMAND:
    offset = 0;
    count = TS_TOTAL_OUTPUT_SIZE;
    // TS will not use this command until ochBlockSize is bigger than
    // blockingFactor and prefer ochGetCommand :(
    cmdOutputChannels(tsChannel, offset, count);
    break;
  ...
  case TS_CHUNK_WRITE_COMMAND:
    ...
    offset = data16[1];
    count = data16[2];
    handleWriteChunkCommand(tsChannel, page, offset, count,
                            data + sizeof(TunerStudioPageRWChunkRequest));
    break;
  ...
  case TS_SIMULATE_CAN:
    void handleWrapCan(TsChannelBase * tsChannel, char *data,
                       int incomingPacketSize);
  ...
  case TS_IO_TEST_COMMAND:
    ...
    // TODO: Why did we process `TS_IO_TEST_COMMAND` only in prod code? I've
    // just turned it on for simulator as well, because 	I need test this
    //functionality with simulator as well. We need to review the cases when we
    //really need to turn off 	`TS_IO_TEST_COMMAND` processing. Do we really need
    //guards below?
    {
  ...
  case TS_SET_LOGGER_SWITCH:
    ...
    case TS_COMPOSITE_READ: {
      ...
      if (toothBuffer) {
        tsChannel->sendResponse(
            TS_CRC, reinterpret_cast<const uint8_t *>(toothBuffer->buffer),
            toothBuffer->nextIdx * sizeof(composite_logger_s), true);
  ...
  default:
    ...
    chsnprintf(tsErrorBuff, sizeof(tsErrorBuff),
...
void startTunerStudioConnectivity() {
  ...
  static_assert(sizeof(persistent_config_s) == TOTAL_CONFIG_SIZE,
...
#if EFI_BLUETOOTH_SETUP
  // module initialization start (it waits for disconnect and then communicates
  // to the module) Usage:   "bluetooth_hc06 <baud> <name> <pincode>" Example:
  // "bluetooth_hc06 38400 rusefi 1234" bluetooth_jdy 115200 alphax 1234
  addConsoleActionSSS(
      "bluetooth_hc05",
      [](const char *baudRate, const char *name, const char *pinCode) {
  ...
  addConsoleActionSSS("bluetooth_bk", [](const char *baudRate, const char *name,
                                         const char *pinCode) {
