...
/*
 * I was a little bit surprised that we declare __attribute__((weak)) before
 * returning type in a function definition. In the most sources this declaration
 * is placed either before function name or after function parentheses, see:
 * -
 * https://gcc.gnu.org/onlinedocs/gcc-4.0.0/gcc/Function-Attributes.html#Function-Attributes
 * - https://en.wikipedia.org/wiki/Weak_symbol
 * But it looks like our manner of __attribute__((weak)) declaration works at
 * well, and I hope it will not cause problems in the future.
 */
void boardSayHello() {
...
static void sayHello() {
  efiPrintf(PROTOCOL_HELLO_PREFIX
            " rusEFI LLC (c) 2012-2024. All rights reserved.");
  efiPrintf(
      PROTOCOL_HELLO_PREFIX " rusEFI v%d@%u now=%d", getRusEfiVersion(),
      /*do we have a working way to print 64 bit values?!*/ (int)SIGNATURE_HASH,
      (int)getTimeNowMs());
  efiPrintf(PROTOCOL_HELLO_PREFIX " Chibios Kernel:       %s",
            CH_KERNEL_VERSION);
  ...
  efiPrintf(PROTOCOL_HELLO_PREFIX " detected HSE clock %.2f MHz PLLM = %d",
            hseFrequencyMhz, autoDetectedRoundedMhz);
...
#define MIN_FLASH_SIZE 512
  ...
  if (ret == 0) {
    efiPrintf("MCU IDCODE %s in %s with %ld KB flash", partNumber, package,
              pnFlashSize);
  } else {
    efiPrintf("MCU IDCODE unknown 0x%lx", DBGMCU->IDCODE);
  }
  efiPrintf("MCU SER_ID %s rev %c",
            (mcuSerId == 0x0d) ? "AT32F435"
                               : ((mcuSerId == 0x0e) ? "AT32F437" : "UNKNOWN"),
            'A' + mcuRevision);
...
#define STM32_RTCSEL_SHIFT 8
    ...
    efiPrintf("STM32_RTCSEL=%d %s actual=%d %s",
              STM32_RTCSEL >> STM32_RTCSEL_SHIFT,
              rtcsel_names[STM32_RTCSEL >> STM32_RTCSEL_SHIFT], rtcsel,
              rtcsel_names[rtcsel]);
#ifdef RUSEFI_STM32_LSE_WAIT_MAX_RTCSEL
    efiPrintf(
        "RUSEFI_STM32_LSE_WAIT_MAX_RTCSEL=%d %s",
        RUSEFI_STM32_LSE_WAIT_MAX_RTCSEL >> STM32_RTCSEL_SHIFT,
        rtcsel_names[RUSEFI_STM32_LSE_WAIT_MAX_RTCSEL >> STM32_RTCSEL_SHIFT]);
#endif
  }
#endif
  ...
  /**
   * Time to finish output. This is needed to avoid mix-up of this methods
   * output and console command confirmation this code here dates back to 2015.
   * today in 2024 I have no idea what it does :(
   */
...
static void cmd_threads() {
  ...
  while (tp) {
    ...
    efiPrintf("%s\t%08x\t%lu\t%d", tp->name, (unsigned int)tp->wabase, tp->time,
              freeBytes);

    if (freeBytes < 100) {
      criticalError("Ran out of stack on thread %s, %d bytes remain", tp->name,
                    freeBytes);
    }
  ...
  int isrSpace =
      CountFreeStackSpace(reinterpret_cast<void *>(&__main_stack_base__));
 ...
 */
static void echo(int value) { efiPrintf("got value: %d", value); }

void checkStackAndHandleConsoleLine(char *line) {
  assertStackVoid("console", ObdCode::STACK_USAGE_MISC,
                  EXPECTED_REMAINING_STACK);
  handleConsoleLine(line);
}

void onCliCaseError(const char *token) {
  firmwareError(ObdCode::CUSTOM_ERR_COMMAND_LOWER_CASE_EXPECTED,
...
void initializeConsole() {
  ...
  addConsoleAction("show_blt_version", []() {
    ...
    efiPrintf("********************** blt=%lx %s version", bltBinVersion,
              bltBinVersion == BLT_CURRENT_VERSION ? "CURRENT" : "UNEXPECTED");
  });
#endif
#if EFI_HAS_RESET
