...
public:
  ...
  CanRxMessage(const CANRxFrame &f) { frame = f; }
...
class IsoTpBase {
public:
  IsoTpBase(ICanTransmitter *p_txTransport, size_t p_busIndex,
            uint32_t p_rxFrameId, uint32_t p_txFrameId)
      : txTransport(p_txTransport), busIndex(p_busIndex),
        rxFrameId(p_rxFrameId), txFrameId(p_txFrameId) {}

  int sendFrame(const IsoTpFrameHeader &header, const uint8_t *data, int num,
...
// todo: no reason for composite entity to exist, keep splitting
// CanStreamerState into RX and TX!
class ICanTransport : public ICanTransmitter, public ICanReceiver {};

// most efficient sizes are 6 + x * 7 that way whole buffer is transmitted as
// (x+1) full packets
...
public:
  CanStreamerState(ICanTransmitter *p_txTransport, ICanReceiver *p_rxTransport,
                   size_t p_busIndex, uint32_t p_rxFrameId,
  ...
  int sendFrame(const IsoTpFrameHeader &header, const uint8_t *data, int num,
                can_sysinterval_t timeout);
  int receiveFrame(const CANRxFrame &rxmsg, uint8_t *buf, int num,
                   can_sysinterval_t timeout);
  int getDataFromFifo(uint8_t *rxbuf, size_t &numBytes);
  // returns the number of bytes sent
  int sendDataTimeout(const uint8_t *txbuf, int numBytes,
                      can_sysinterval_t timeout);

  // streaming support for TS I/O (see tunerstudio_io.cpp)
  can_msg_t streamAddToTxTimeout(size_t *np, const uint8_t *txbuf,
                                 can_sysinterval_t timeout);
  can_msg_t streamFlushTx(can_sysinterval_t timeout);
  can_msg_t streamReceiveTimeout(size_t *np, uint8_t *rxbuf,
                                 can_sysinterval_t timeout);
};
...
public:
  ...
  ~IsoTpRx() { unregisterCanListener(*this); }
