...
void setHpfpLobeProfileAngle(int lobes) {
  ...
  static const float hardCodedHpfpLobeProfileAnglesForThreeLobes[16] = {
      0.0,  7.5,  16.5, 24.0, 32.0, 40.0,  48.0,  56.0,
      64.0, 72.0, 80.0, 88.0, 96.0, 103.5, 112.5, 120.0};

  float multiplier = 3.0 / lobes;
  for (size_t i = 0; i < HPFP_LOBE_PROFILE_SIZE; i++) {
    config->hpfpLobeProfileAngle[i] =
        multiplier * hardCodedHpfpLobeProfileAnglesForThreeLobes[i];
  }
...
#if !EFI_UNIT_TEST
  // unit tests rely on 'hpfpCamLobes' for isGdiEngine() and we need not-GDI by
...
void defaultsOrFixOnBurn() {
  ...
  if (engineConfiguration->mapExpAverageAlpha <= 0 ||
      engineConfiguration->mapExpAverageAlpha > 1) {
    engineConfiguration->mapExpAverageAlpha = 1;
  }

  if (engineConfiguration->ppsExpAverageAlpha <= 0 ||
      engineConfiguration->ppsExpAverageAlpha > 1) {
    engineConfiguration->ppsExpAverageAlpha = 1;
  }
  if (engineConfiguration->afrExpAverageAlpha <= 0 ||
  ...
  if (engineConfiguration->vvtControlMinRpm <
...
void setDefaultBaseEngine() {
  ...
  engineConfiguration->calibrationBirthday =
      compilationYear() * 10000 + compilationMonth() * 100 + compilationDay();
  ...
  setLinearCurve(engineConfiguration->tractionControlSlipBins, /*from*/ 0.9,
                 /*to*/ 1.2, 0.05);
  setLinearCurve(engineConfiguration->tractionControlSpeedBins, /*from*/ 10,
                 /*to*/ 120, 5);
  ...
  // todo: this "2JZ" trigger is very powerful for many low tooth quantity
  // applications todo: we might be getting closer to a re-name by the way 2GRFE
  // intake likes position 160 / precision 20 see also
  // https://github.com/rusefi/rusefi/issues/7345
  //
  // 2JZ values
  ...
  engineConfiguration->vvtPins[0] =
      Gpio::A0; // a random unused pin needed to unblock startSimplePwmExt()
#endif          // EFI_SIMULATOR
  ...
  engineConfiguration->idleRumbleMaxTps = 2;
...
void setTPS1Calibration(uint16_t tpsMin, uint16_t tpsMax,
                        uint16_t tps1SecondaryMin, uint16_t tps1SecondaryMax) {
...
void setCustomMap(float lowValue, float mapLowValueVoltage, float highValue,
...
void setPPSCalibration(float primaryUp, float primaryDown, float secondaryUp,
                       float secondaryDown) {
...
void setupTLE9201IncludingStepper(Gpio controlPin, Gpio direction, Gpio disable,
                                  int dcIndex) {
  setupTLE9201(controlPin, direction, disable, dcIndex);

  // on SBC style stepper IAC fully-extended valve shaft would give least idle
  // air fully-retracted valve shaft would give most idle air
  int stepperIndexWeirdness = 1 - dcIndex;
  engineConfiguration->stepperDcIo[stepperIndexWeirdness].controlPin =
      controlPin;
  engineConfiguration->stepperDcIo[stepperIndexWeirdness].directionPin1 =
      direction;
  engineConfiguration->stepperDcIo[stepperIndexWeirdness].directionPin2 =
      Gpio::Unassigned;
